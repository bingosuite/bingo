...
- If the debugger's runtime scan itself hit its ceiling, the totals are a lower
  bound: the count is marked with a trailing `+` and the note reads
  "the debugger stopped after finding N …, so more may exist".
- An empty tree is attributed to whichever cause the totals actually support —
  "the debugger found N live goroutines, but none fit within the snapshot
  limits" is a different problem from a runtime it could not read, and saying
...
- The WebSocket transport limit sits deliberately **above** the decoder limit, so
  ...
  instance — stays recoverable rather than ending the view. **Refresh** is the
  manual recovery for every terminal state — a protocol error or an exhausted
  reconnect ladder — and redials rather than asking a socket that is gone.
- Only the two bounded events can produce a terminal protocol error from their
  size. A long `Error` message or a deep `Frames` list is legal server output, so
  the view rejects that one frame and reconnects rather than stopping.
