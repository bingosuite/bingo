...
**Size budget — scoped to this event family only (1.4, issue #194).** Both
producers pack: `goroutineSnapshot()` and `readGoroutines()` in
[goroutines.go](internal/debugger/goroutines.go) call `protocol.PackSnapshot` /
`protocol.PackGoroutines` before the event leaves the engine, and
`Debugger.Goroutines()` returns the packed `GoroutinesPayload` so the hub
forwards it verbatim rather than rewrapping and dropping `Totals`. Each scan
reports its OWN clipping — `buildGoroutineList` when it hits `maxGoroutineScan`,
`readThreads` when it hits `maxThreadScan` — because the ceilings are
independent. The lifecycle deltas are computed against the FULL scan before
packing; deriving them from a truncated list would invent exits for goroutines
that merely did not fit. These are
