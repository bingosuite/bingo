...
**Size budget — scoped to this event family only (1.3, issue #194).** These are
...
Both sides testing that rule *in their own terms* is how they would drift, so
the boundary cases live in ONE fixture both languages read:
...
`encoding/json` produced. Neither side can move the boundary alone.
...
**Producer wiring — the contract is in force at the source.** Every producer of
either bounded event packs, so no unbounded goroutine event can reach the wire.
`goroutineSnapshot` (backing `emitBreakpointHit`, `emitPaused`,
`emitGoroutineSnapshot`, and the on-demand `GoroutineSnapshot`) calls
`PackSnapshot`; `readGoroutines` (backing `Goroutines` and, through it, the DAP
threads list) calls `PackGoroutines`. Four properties are load-bearing:

- **Pack AFTER lifecycle accounting.** `diffGoids` runs on the goroutines that
  were actually *alive*, not on the subset that survived the byte budget. Packing
  first would make every trimmed goroutine look like it had exited and then been
  created again on the next stop — a lifecycle stream that is wrong precisely on
  the busy targets this contract exists for.
- **The clipped flags come from the walks themselves.**
  `buildGoroutineList` reports that `runtime.allgs` outran `maxGoroutineScan`;
  `readThreads` reports that the `runtime.allm` walk ended with the list still
  going (its ceiling, or an unreadable `alllink`). They stay independent all the
  way to `SnapshotTotals`.
- **The stop event's goroutine is resolved from the UNPACKED set.**
  `snapshotResult` carries it beside the packed payload, so a
  `BreakpointHit`/`Paused` still names where the tracee is even if the snapshot
  event had to degrade. Those payloads carry one goroutine and are not part of
  the bounded family.
- **A non-conforming result is never broadcast.** `Oversized` means not even
  empty collections fit, which requires lifecycle deltas beyond
  `MaxLifecycleDeltaIDs` — impossible while `maxGoroutineScan` bounds them (a
  producer spec pins that relation). If it ever happened, the streamed path drops
  the event and logs, and the on-demand path returns an error the hub turns into
  `EventError`: a conforming consumer treats a contract violation as
  deterministic and stops observing, so emitting one would cost the whole
  session, while an error merely resolves the caller's wait.

`Debugger.Goroutines` and `client.Client.Goroutines` therefore return
`GoroutinesPayload`, not a bare slice — dropping `Totals` at the hub or in the
reference client would put a truncated list on screen with nothing to say so,
...
**Fatality is scoped to what the server actually promised.** Only a bounded-kind
payload, or a structural/envelope/version violation, is a proven contract breach
and terminal. The bounded family's size caps are still applied to every consumed
kind as this process's own defence, but exceeding them on an UNBOUNDED kind
(a long `Error` message echoing a pasted Watch expression, a deep `Frames` list)
throws an ordinary error so the reconnect ladder still applies. Latching there
would kill the view for the rest of the session over output the server was
entitled to send — the #194 failure mode wearing different clothes.
...
every kind (exact keys, `v == 1.3`, known kind, `seq >= 1`, object payload). But
...
`refresh()` is the manual recovery for EVERY terminal state — a fatal protocol
latch or an exhausted reconnect ladder — and redials whenever no socket is left,
because re-sending a snapshot request over a socket that is gone silently
strands the panel. While no snapshot has arrived the view names the connection
state it is actually in; claiming "Connecting" after the observer stopped makes
the panel look busy, so nobody presses the one control that would recover it.
An empty tree or thread list is attributed to omission rather than a failed
runtime read whenever `Totals` proves the debugger had the data.
...
- **Wire protocol** (`pkg/protocol`): bump `Version` for breaking changes,
  and update the round-trip table in `protocol_test.go`. A bump must land in the
  SAME change as the producers and consumers that honour it — advertising a
  contract the server violates is worse than not having one, and so is enforcing
  one the server does not yet emit. Currently **1.3** (the
  goroutine event family's size contract: `MaxGoroutineEventBytes` /
  `MaxSnapshotGoroutines` / `MaxSnapshotThreads` / `MinThreadsRetained`, the new
  `SnapshotTotals` on both `GoroutineSnapshotPayload` and `GoroutinesPayload`,
  the `PackSnapshot` / `PackGoroutines` packers, both `internal/debugger`
  producers wired to them, and `Goroutines` returning `GoroutinesPayload` in both
  `debugger.Debugger` and `client.Client`). 1.2 had the reshaped
...
- **Goroutine event size contract**: any new producer of
  `EventGoroutineSnapshot`/`EventGoroutines` must pack, and any change to
  `maxGoroutineScan`/`maxThreadScan` must keep `maxGoroutineScan <=
  MaxLifecycleDeltaIDs` or the deltas can overflow a contract nothing can then
  trim. Moving `MaxGoroutineStringLength` means regenerating
  [the shared boundary fixture](pkg/protocol/testdata/goroutine_string_boundary.json)
  (`BINGO_UPDATE_FIXTURES=1`) and re-running the VS Code suite in the same
  change, since both languages enforce it against that one file.
