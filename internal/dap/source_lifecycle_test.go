...
func TestSourceBuildRemainsResponsiveAndRejectsLateSuccess(t *testing.T) {
	...
	requireSourceStartupRejections(t, hh, dir)
	for range 2 {
...
func requireSourceStartupRejections(t *testing.T, hh *harness, dir string) {
	t.Helper()
...
func TestSourceArtifactsFollowHubLifetimeAcrossRestartAndObservers(t *testing.T) {
	...
	requireSourceRestartOverrides(t, third, artifact.program, dir, builds.Load())
	_ = hh.client.Close()
...
func requireSourceRestartOverrides(t *testing.T, got protocol.LaunchPayload, program, cwd string, buildCount int32) {
	t.Helper()
	if got.Program != program || got.Cwd != cwd || got.Args == nil || len(got.Args) != 0 ||
		got.Env == nil || len(got.Env) != 0 || buildCount != 1 {
		t.Fatalf("restart overrides rebuilt or lost cwd/nil semantics: %+v, builds=%d", got, buildCount)
	}
}
