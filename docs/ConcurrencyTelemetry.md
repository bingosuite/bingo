...
- Both observers report scale honestly. `wsmon`'s `counts:` line shows
  ...
  `N goroutines omitted from this event` (the packer left elements off the wire)
  and `the debugger stopped after finding N …, so more may exist`, each with a
  trailing `+` on its own count (that runtime walk stopped early, so that total
  is itself a floor). When the payload carries no totals the line says
  `nothing omitted`, which is precisely what their absence proves — it is NOT a
  claim that the debugger saw the whole runtime, since a degraded read reports
  one synthetic goroutine and omits nothing.
